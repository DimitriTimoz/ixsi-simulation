...
pub fn get_recos_and_random_users() -> (BTreeMap<Movie, HashSet<usize>>, Vec<UserRatings>) {
    // Read the lines
    ...
    // Ids of users to be selected pick 100 random users
    let mut users = (0..n_users).map(|_| rng.gen_range(0..200_000)).collect::<HashSet<usize>>();
    let mut users: HashMap<usize, HashSet<Movie>> = HashMap::from_iter(users.into_iter().map(|user_id| (user_id, HashSet::new())));
    ...
    for line in file.lines().skip(1)  {
        ...
        // 
        let user_ratings = reviews.entry(movie).or_insert(HashSet::new());
        user_ratings.insert(user_id);

        // Add the movie to the user's list of movies if is selected
        if let Some(view_movies) = users.get_mut(&user_id) {
            view_movies.insert(movie);
            users.insert(user_id, view_movies.clone());    
        }
     
    }

    (reviews, users.into_iter().map(|(user_id, ratings)| UserRatings { user_id, ratings }).collect())
