...
async fn main() {
    let (recos, users, picked_users) = movies::get_recos_and_random_users();
    ...
    for user in &picked_users {
        ...
        let query = RecoQuery::from(user.ratings.iter().take((user.ratings.len() as f32 * 0.9) as usize).cloned().collect::<Vec<Movie>>());
